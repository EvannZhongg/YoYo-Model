...
- 悠悠球检测：YOLO11s，`confidence=0.15`、`iou=0.70`、`imgsz=1024`；只执行显式关闭增强的常规推理。五段审核序列表明增强二次推理降低 F1 和速度，该运行分支及其配置已移除。
...
- 每帧只运行当前主模型和固定副模型两次语义前向；两者共享一次 letterbox、归一化和 GPU 输入张量，三个 checkpoint 常驻显存。`string_inference_fps=0` 表示每帧重建绳线观测，门控也只在实际语义推理帧更新。
