...
**结论**：单帧检测的低置信度候选不适合直接降低全局阈值，但在上一帧可信且空间连续的条件下，可作为受控掉检救援，改善连续视频召回。

**证据**：当前 YOLO11s replay+soup 权重、`imgsz=1024`、主阈值 `0.15` 的离线 10 组/927 帧扫描中，主路径 presence F1 为 `0.977387`（TP/FP/FN=`778/9/27`）；仅在可信历史框附近接纳 `conf=0.03` 候选（归一化中心距离≤`1.5`）后，离线 F1 为 `0.980613`（`784/10/21`）。完整 10 组真实视频回放得到 pooled presence `0.984962/0.976398/0.980661`（P/R/F1，TP/FP/FN/TN=`786/12/19/3`），mean/median IoU `0.802578/0.846517`，最长缺失/恢复 `6/6`，平均检测循环 `10.63 FPS`。最弱来源为邬聪聪（F1 `0.923077`）；该结果没有牺牲整体吞吐门槛，但弱来源仍需继续补数据。

**适用范围**：YOLO11s replay+soup、`1Ayoyo_consecutive` 当前 10 组标注、主阈值 `0.15`、低阈值 `0.03` 和单帧空间门控；离线扫描不替代完整端到端全量回放。

**后续建议**：保留该单候选时序救援作为当前追踪默认路径；下一轮用完整 10 组端到端回放更新 pooled 指标与 FPS，并继续监控多悠悠球场景的轨迹身份稳定性。不要将 `0.03` 直接设为全局检测阈值。

## 邬聪聪来源组 HD95 异常的逐帧定位

**结论**：更新标注候选在邬聪聪来源组的 HD95 回退主要由连续远端背景误检造成，而不是少数孤立帧或单纯的骨架碎片化。误检在画面右侧延伸到 `x≈3815`，真实绳线通常集中在悠悠球/手部附近；双向最近距离的 95 分位数因此被大幅抬高。

**证据**：候选 `tmp/semantic_updated_consecutive/邬聪聪-0d26cf65b6--run-00006200-00006399-8f847874.metrics.json` 共 94 个有目标中心线的帧，HD95 均值为 `1310.83 px`。`6219–6245` 共 27 帧平均 HD95 `1967.8 px`，`6251–6262` 共 12 帧平均 `1896.4 px`；两段合计 39 帧，占该组 HD95 总和约 `61.6%`。最高帧为 `6232/6233/6234/6235/6245`，HD95 分别为 `2045.2/2035.1/2021.4/2019.4/2015.1 px`。其中 `6223–6225` 的 F1@8 为 `0`，目标线约 `77–169 px`，预测线约 `310–335 px` 且主要位于错误的远端区域；`6251–6262` 还表现为目标线缩短至 `10–142 px` 而预测仍保持 `285–517 px`。逐帧 F1、Chamfer、长度和组件数见同一 metrics JSON；对应原图位于 `datasets/1Ayoyo_consecutive/canonical/images/邬聪聪-0d26cf65b6/`。

**适用范围**：当前更新标注候选 `semantic_updated_labels_prod_e12`、`1Ayoyo_consecutive` 邬聪聪 `6200–6298` 片段、`1.125x` 输入及颜色/亮线/时序后处理；HD95 按现有双向采样距离实现计算。

**后续建议**：后续 A/B 优先检查 `6219–6245` 和 `6251–6262` 的远端组件来源与空间约束，分别统计移除远端组件前后的 HD95、F1@8 和 Presence；不要只按平均 HD95 调阈值，也不要将单个异常帧当作随机噪声。处理后仍需在邬聪聪、池高宇及完整连续集上复核最弱组、Presence 和最长缺失/恢复护栏。
