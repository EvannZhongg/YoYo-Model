...
**结论**：关闭 YOLO11s 训练期 `scale` 增强未改善当前小球检测；连续集低阈值召回和几何质量略降，弱来源回退明显，因此保留生产 `scale=0.15`。

**证据**：同一 `90f330d7...` manifest、YOLO11s、`imgsz=1024`、seed `20260911`、8 epoch foundation 训练下，`scale=0` 候选 native test mAP50-95 为 `0.584538`，低于生产 `0.586875`。连续集在 `conf=0.03` 下 Presence F1 / mean IoU / 最长缺失为 `0.979180 / 0.796290 / 5`，弱来源“邬聪聪” F1 为 `0.871795`；生产同协议为 `0.980661 / 0.802578 / 6`，弱来源 F1 `0.923077`。

**适用范围**：当前 `yoyo_unified_579b879a66ce` manifest、YOLO11s、固定 `1024` 输入和 `1Ayoyo_consecutive` 927 帧回放；只改变训练尺度增强，未改变网络结构、输入或后处理。

**后续建议**：默认保留 `scale=0.15`；若要改善远距离小球，应优先补充真实弱域标注，再验证增强策略，不能以关闭尺度增强替代数据覆盖。
