...
## 2026-09-08 方向三/四分类复核

**结论**：将 `presentation_orientation` 拆成四类并训练独立 ROI 分类器，经过现有自适应采样、EMA 和滞回滤波后，连续帧 pooled 指标与三分类接近，但最弱来源组仍明显回退；test 上不足一个百分点的投影增益不足以抵消该风险，因此当前不晋升四分类模型。

**证据**：同一 `yoyo_unified_5673a7faf873` 来源隔离 test 上，现有三分类 top-1 为 94.08%，四分类 top-1 为 90.13%，四类投影后三分类准确率为 94.74%。在 `1Ayoyo_consecutive` 按来源组、帧序运行现有滤波协议后，三分类 pooled accuracy/macro-recall 为 96.01%/86.31%，四分类为 97.09%/86.33%，但最弱来源组分别为 90.91% 与 84.85%。未滤波逐帧结果中四分类投影为 85.33%，说明收益依赖时序滤波。四分类训练集仅对 train 做类别平衡，val/test 未改变，权重与 manifest 均独立记录。

**适用范围**：当前 1A canonical ROI 数据、YOLO11n-cls、320 输入、30 epoch；连续帧数据不参与训练，仅作弱域验证。

**后续建议**：若新增带可靠 `edge_vertical` 标注的来源，应重新验证四分类是否能提升弱域；在此之前继续使用三分类默认权重，不以 test 上不足 1 个百分点的投影增益替换生产模型。

## 2026-09-08 backbone 对照

**结论**：在当前 795 张训练图（仅训练集重复平衡）和 320 输入下，增大到 YOLO11s 没有带来方向识别收益，不能以更高参数量换取当前数据规模上的提升。

**证据**：同一 manifest、相同 30 epoch/seed/输入协议下，三分类 YOLO11n test top-1/macro-recall 为 94.08%/95.68%，YOLO11s 为 90.79%/94.37%；四分类 YOLO11n 为 90.13%/81.90%，YOLO11s 为 88.16%/74.34%。YOLO11s 约 5.44M 参数、12.0 GFLOPs，YOLO11n 约 1.53M 参数、3.2 GFLOPs，且更慢更大。

**适用范围**：当前 1A canonical ROI 数据、YOLO11 分类预训练权重、320 输入和 30 epoch 训练协议。

**后续建议**：除非数据规模或标注覆盖显著增加，否则优先优化输入裁剪、标注质量和时序策略，不继续堆叠 backbone 容量。
