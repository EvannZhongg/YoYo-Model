...
**证据**：RTX 4070 Laptop、PyTorch 2.11、`960x544` 输入的前向均值由 `6.918 ms` 降至 `6.543 ms`（约 `5.4%`），同一输入和第二张不同输入的最大绝对概率差均为 `0`。邬聪聪 300 帧正式追踪中，Graph/eager 两组逐帧 JSONL SHA-256 均为 `E654B144AFF71A1D7EFC88DE60DCF5772F53B54B244DBB689961C13F74E81990`；两次相邻 A/B 的 FPS 为 `10.8914/10.6918` 与 `10.8439/10.9713`，平均差异约 `+0.3%`。
