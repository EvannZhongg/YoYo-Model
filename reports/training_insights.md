...
**证据**：当前 YOLO11s replay+soup 权重、`imgsz=1024`、主阈值 `0.15` 的离线 10 组/927 帧扫描中，主路径 presence F1 为 `0.977387`（TP/FP/FN=`778/9/27`）；仅在可信历史框附近接纳 `conf=0.03` 候选（归一化中心距离≤`1.5`）后，F1 为 `0.980613`（`784/10/21`），最长缺失段由 7 降至 6。真实 `namdongxun` 3121–3225 片段 105 帧回放中，presence F1 为 `1.0000`（TP/FP/FN=`105/0/0`），基线为 `0.9286`（`91/0/14`），最长缺失由 6 降至 0。
