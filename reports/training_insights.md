...
**结论**：在当前来源隔离 manifest 上，从 ImageNet foundation 重新训练三分类 ROI 模型，能同时改善独立 test 的总体准确率、少数类召回和连续帧稳定性；无需增加模型容量或额外门控，当前晋升为默认方向权重。

**证据**：`yoyo_unified_5673a7faf873` 的 798 张 train 样本经类内重复采样形成 2031 张均衡训练视图，YOLO11n-cls、`320 px`、batch 32、seed `20260905`、最多 30 epoch 的 native 152 张 test Top-1/Macro Recall 为 `0.940789/0.956780`；旧生产权重在同一 test 为 `0.842105/0.827909`。候选三类召回为 `0.933333/0.937008/1.000000`。在 927 帧连续集和相同时序协议下，候选/旧权重 Accuracy 为 `0.960086/0.908306`，Macro Recall 为 `0.863079/0.859252`，预测切换数为 `10/13`，最弱组由 `0.611111` 提升至 `0.861111`；两个来源组分别回退 `0.021053` 和 `0.081633`。同一 RTX 4070 配对测试约为 `431/456 FPS`，候选吞吐下降约 `5.5%`。

**适用范围**：当前三分类 ROI 裁剪、来源隔离的 798/151/152 canonical split、YOLO11n-cls 和现有 5/25 FPS 自适应时序滤波；test 的少数类样本仍只有 15 张 horizontal 和 10 张 not_applicable。
...
**结论**：在当前扩展 reviewed manifest 上从 ImageNet backbone 完整训练 30 epoch 的 MobileNetV3-FPN，静态独立 test 和连续集 pooled 几何指标均优于生产模型，但默认验证阈值下弱来源与时序可靠性回退，暂不晋升。
...
固定生产阈值 `0.9204` 的敏感性复核将连续集 pooled F1@8 提升到 `0.849603`，最弱组为 `0.639368`，但 Presence F1 和最长缺失/最大恢复仍为 `0.991247`、`4/4`，低于生产的 `0.994530`、`2/2`；同阈值独立 test centerline F1@8 为 `0.892983`，负图平均误检 `32.273`。因此阈值校准可以修复弱组 F1 的局部回退，却不能满足时序安全护栏。
