...
| 绳线分割 | 三权重 LR-ASPP 路由 | 普通域双路融合，弱域按门控置信度校准，超弱域单主模型 |
...
- 低置信一般弱域：触发窗口平均 confidence `<0.74` 时，把弱域主模型的校准原点从 checkpoint 原生 `0.2991` 调整为 `0.32`；其他一般弱域保留原生校准。
...
| 池高宇，67 帧 | 67 | 0.765255 | 0.282884 | 0.413072 | 64.7856 | 1.000000 |
| 池高宇前场 | 120 | 0.726775 | 0.413123 | 0.526797 | 24.0786 | 1.000000 |
...
Pooled Precision/Recall/F1@8 为 `0.711532/0.443424/0.546359`，帧均 Chamfer/HD95 为 `88.2903/195.6766 px`。相对上一生产策略，三项 P/R/F1 均提升，Chamfer `88.4406 -> 88.2903`，HD95 `195.7562 -> 195.6766`；除上表两个池高宇区间外，其余 6 组逐帧 JSONL 的 SHA-256 完全一致。最终结果位于：

`runs/experiments/round_adaptive_calibration_t032_conf074_production_757/summary.json`
...
低置信一般弱域校准只命中两个池高宇区间：67 帧区间 F1@8 `0.405558 -> 0.413072`、Chamfer `65.4543 -> 64.7856 px`；120 帧区间 F1@8 `0.514192 -> 0.526797`、Chamfer `24.6535 -> 24.0786 px`。高置信 `namdongxun` 保留原生阈值，因此避免了全局调整产生的回退。
...
| 门控后再创建弱域 CUDA Graph | 超弱单路不再创建不会使用的双路预测器；不增加模型或前向 |
...
最新校准策略又在 `namdongxun` 的 105 帧真实 detector 驱动 Workbench A/B 中验证：新旧输出 MP4 SHA-256 均为 `4f0e620e18da3cdab68e4f3a768ca8c462aa493f31fe778f183d0bc8a256b914`，两种执行顺序下新策略都更快（`7.8194 -> 8.4648 FPS`、`8.3076 -> 9.6136 FPS`）。样本次数不足以宣称稳定加速，但可以确认没有输出或速度回退。
