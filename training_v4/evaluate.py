...
def evaluate(weights: str | Path, dataset_dir: str | Path, split: str = "test", device_name: str = "cuda", thresholds: list[float] | None = None, max_samples: int | None = None) -> dict[str, Any]:
    ...
    return {"schema_version": "yoyo_training_v4_eval_v2", "task": "mask_centerline_tangent_fusion", "weights": str(Path(weights).resolve()), "weights_sha256": sha256_file(Path(weights)), "dataset_dir": str(root), "split": split, "dataset_manifest_sha256": sha256_file(root / "manifest.json"), "summary": summary, "samples": len(rows), "rows": rows}
