#!/usr/bin/env python3
"""Create a deduplicated blank yoyo/string dataset for the local Workbench."""
...
def descriptor(image: Image.Image) -> np.ndarray:
    small = np.asarray(image.resize((24, 14), Image.Resampling.BILINEAR), dtype=np.float32)
    gray = np.asarray(image.convert("L").resize((16, 9), Image.Resampling.BILINEAR), dtype=np.float32).reshape(-1)
    histograms = [np.histogram(small[:, :, channel], bins=8, range=(0, 256))[0] for channel in range(3)]
    features = np.concatenate([*(item.astype(np.float32) for item in histograms), gray])
...
def candidate_indices(frame_count: int, desired: int, oversample: int, edge_fraction: float) -> list[int]:
    ...
    count = min(stop - start, max(desired * oversample, desired + 16))
    if count <= 1:
        return [start]
    return sorted({int(round(start + index * (stop - start - 1) / (count - 1))) for index in range(count)})
...
def select_candidates(
    ...
    used: set[int] = set()
    for stratum in range(desired):
        start = math.floor(stratum * frame_count / desired)
        stop = math.floor((stratum + 1) * frame_count / desired)
        pool = [
            item for item in candidates
            if start <= item.frame_index < stop
            and item.frame_index not in used
            and all(abs(item.frame_index - chosen.frame_index) >= min_gap for chosen in selected)
            and not is_perceptual_duplicate(
                item.difference_hash,
                (chosen.difference_hash for chosen in selected),
        ...
        def score(item: Candidate) -> tuple[float, float, int]:
            diversity = min(
                (1.0 - float(np.clip(np.dot(item.descriptor, chosen.descriptor), -1.0, 1.0)) for chosen in selected),
                default=1.0,
            )
            return diversity, -abs(item.frame_index - center), -item.frame_index

        chosen = max(pool, key=score)
        selected.append(chosen)
        used.add(chosen.frame_index)
    remaining = sorted(candidates, key=lambda item: item.frame_index)
    while len(selected) < desired:
        ...
        chosen = max(
            pool,
            key=lambda item: min(
                (1.0 - float(np.clip(np.dot(item.descriptor, prior.descriptor), -1.0, 1.0)) for prior in selected),
                default=1.0,
            ),
        )
        selected.append(chosen)
...
def validate_staged_dataset(
    ...
    for label_path in labels:
        ...
        if is_perceptual_duplicate(dhash, seen_dhashes, perceptual_threshold):
            raise ValueError(f"within-batch perceptual overlap survived generation: {image_path}")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    if args.min_frame_gap < 1 or args.exclude_frame_window < 0:
        parser.error("min-frame-gap must be positive and exclude-frame-window cannot be negative")
